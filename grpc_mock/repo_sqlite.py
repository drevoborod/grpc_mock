...
class MockRepoSqlite(_RepoSqlite, MockRepo):
    async def get_grpc_mocks_from_storage(
        ...
        async with self.db.execute(
            ...
            f"from {TableNames.GRPC_MOCKS} where package_name=? and service_name=? "
    ...
    async def update_mock(
        ...
        await self.db.executemany(
            f"update {TableNames.GRPC_MOCKS if mock_type == MockType.grpc else TableNames.REST_MOCKS} "
            "set is_deleted=?, updated_at=? where id=?",
...
class LogRepoSqlite(_RepoSqlite, LogRepo):
    async def get_grpc_log(
        ...
        clause = " and ".join([f"{TableNames.GRPC_MOCKS}.{key}=?" for key in query_params])
        if clause:
            clause = f" where {clause}"
        async with self.db.execute(
            f"select {TableNames.GRPC_MOCKS}.config_uuid, {TableNames.GRPC_LOGS}.request, {TableNames.GRPC_LOGS}.response, "
            f"{TableNames.GRPC_LOGS}.response_status, {TableNames.GRPC_LOGS}.created_at from {TableNames.GRPC_MOCKS} "
    ...
    async def get_rest_log(
        ...
        params = locals()
        query_params = {param: params[param] for param in ("endpoint", "method", "config_uuid") if params[param]}
        clause = " and ".join([f"{TableNames.REST_MOCKS}.{key}=?" for key in query_params])
        if clause:
            clause = f" where {clause}"
        async with self.db.execute(
                f"select {TableNames.REST_MOCKS}.config_uuid, {TableNames.REST_LOGS}.request, {TableNames.REST_LOGS}.response, "
                f"{TableNames.REST_LOGS}.response_status, {TableNames.REST_LOGS}.created_at from {TableNames.REST_MOCKS} "
