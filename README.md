...
* web application backend consisting of several microservices which communicate each other using gRPC over HTTP 2.0 or HTTP 1.1 with REST.
* some of them require to be mocked because they aren't required for testing purpose (or it can be external system which should not be accessed during testing process.)
...
2. Before the test begins (during the setup process) mock service is configured by REST request to response to specific gRPC or REST request by sending provided data (using POST /grpc_mock or /rest_mock endpoints).
...
Actual configuration endpoints are listed in `docs/openapi/swagger.yaml`.
